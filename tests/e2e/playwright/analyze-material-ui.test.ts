...
const TEST_PAGE_URL = `${TEST_SERVER_URL_FOR_API}/fixtures/material-ui.html`;

test.describe('Material-UI Component Detection', () => {
  test('should detect Material-UI components and extract design tokens', async ({ request }) => {
    const siteName = `test-material-ui-${Date.now()}`;
    ...
    // Verify Material-UI patterns were detected
    expect(designSystem.metadata.name).toBe(siteName);
    
    // Check for Material-UI-specific patterns
    const hasMaterialUIPatterns = 
      designSystem.component && 
      Object.keys(designSystem.component).length > 0;

    expect(hasMaterialUIPatterns || designSystem.libraryPatterns?.includes('material-ui')).toBeTruthy();
  });
});
