...
**Created**: 2025-11-13  
**Status**: Draft  
**Input**: User description: "Ты — инженер-разработчик. Создай полноценный микросервис на Node.js/TypeScript, который состоит из двух частей:
...
- **FR-028**: System MUST be deployed using Docker containers via docker-compose.yml, with at minimum: (1) primary Node.js service container containing HTTP API server, MCP server, and background job workers, (2) test container for running automated tests. Additional worker containers with Redis MAY be added for independent scaling if needed.
